...
- 톨킨 스타일의 중세 판타지
- 어둠의 세력에 맞서는 영웅의 여정
- 한국어 서비스, 문체는 고풍스럽고 서사적인 톤

### 핵심 게임플레이
1. 플레이어가 텍스트로 행동 입력 ("북쪽 문을 연다", "상인과 대화한다")
2. Codex가 게임 마스터(GM) 역할로 결과 내레이션 생성
...
const gameState = {
  ...
  turn: 0,
  history: []           // 대화 히스토리 (Codex API messages 배열)
};
```
...
ai-trpg/
├── .Codex/
│   └── AGENTS.md          ← 이 파일 (Codex 컨텍스트)
├── public/                ← Express가 정적 파일로 서빙
│   ├── index.html         ← 게임 메인 화면
│   ├── style.css          ← 스타일
│   └── script.js          ← 프론트엔드 전체 로직 (현재 단일 파일)
├── server.js              ← Express 서버 + Codex API 프록시
├── package.json
...
- [x] Express 서버 + Codex API 연결 구조 완성
...
- Codex API 응답이 JSON이 아닐 경우 재시도 또는 오류 안내 처리 필요
