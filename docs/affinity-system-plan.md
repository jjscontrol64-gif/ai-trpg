...
에이미·실루엘라와의 **호감도(Affinity)** 시스템을 추가한다. 플레이어는 ⛺ 안전지대에서 동료와 대화하며 호감도를 쌓고, 호감도는 평소 대화의 분위기와 최종 엔딩에 영향을 준다.
...
| 2 | **단계** | 캐릭터별 **3단계** (`0` / `1` / `2`), 초기값 `0`. 에이미·실루엘라 **독립** 관리 |
...
⛺ 안전지대 진입
  ...
  → phase = "event"
  → 선택지 노출:
       affinity_talk(pina)     "에이미와 대화"
       affinity_talk(mina)     "실루엘라와 대화"
...
- 호감도 단계에 따라 **잠금 해제되는 엔딩 선택지**를 제시 (예: 특정 동료와의 후일담 장면).
  - 예: 에이미 2단계 → "에이미와의 약속" 선택지 노출 / 실루엘라 2단계 → "실루엘라와의 약속" 선택지 노출.
  - 둘 다 낮으면 공통 마무리 선택지만.
